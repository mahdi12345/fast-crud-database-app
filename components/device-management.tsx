...
export default function DeviceManagement({ devices }: DeviceManagementProps) {
  ...
  const getDeviceIcon = (browserInfo: any) => {
    if (!browserInfo) return <Monitor className="h-4 w-4" />

    let userAgent = ""
    if (typeof browserInfo === "string") {
      try {
        const parsed = JSON.parse(browserInfo)
        userAgent = parsed.userAgent?.toLowerCase() || ""
      } catch {
        userAgent = browserInfo.toLowerCase()
      }
    } else if (typeof browserInfo === "object" && browserInfo.userAgent) {
      userAgent = browserInfo.userAgent.toLowerCase()
    }
  ...
  const getDeviceInfo = (browserInfo: any) => {
    ...
    try {
      ...
      } else if (typeof browserInfo === "object") {
